...
// Request logging: every request that hits the backend is written to
// stdout in the standard "combined" access-log format. Since containers'
// stdout is what Alloy/Loki collect, this alone makes every request
// visible in Grafana without any extra plumbing.
...
app.post("/todos", async (req, res) => {
  ...
  // Log every todo that is sent to the backend, so it's visible in
  // Grafana/Loki regardless of whether it ends up being accepted.
  console.log(`Received todo submission (${typeof todo === "string" ? todo.length : 0} chars): ${JSON.stringify(todo)}`);

  if (!todo || typeof todo !== "string" || todo.trim().length === 0) {
    console.warn("Rejected todo: empty or not a string");
    return res.status(400).json({ error: "todo is required" });
  }

  if (todo.length > TODO_MAX_LENGTH) {
    console.warn(
      `Rejected todo: length ${todo.length} exceeds TODO_MAX_LENGTH (${TODO_MAX_LENGTH})`
    );
  ...
  try {
    ...
    const { rows } = await pool.query("SELECT text FROM todos ORDER BY id");
    console.log(`Todo accepted and saved (${todo.length} chars)`);
